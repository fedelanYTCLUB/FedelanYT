...
let handler = async (m, { conn, usedPrefix, text, args, command }) => {
    ...
    const nomorown = '51939508653' 
    const dev = 'Colaborador del bot' 
    const correo = 'xxxxxxxxxxxxx' 
    const packname = '💙Hatsune Miku Bot💙' 
    const md = 'https://github.com' 
    const global = { yt: 'https://youtube.com' } 

    await sendContactArray(conn, m.chat, [
        [`${nomorown}`, `💙 Propietario`, `Rafael`, dev, correo, `BVH3`, `${global.yt}`, bio],
        [`${conn.user.jid.split('@')[0]}`, `Es Una Bot 💙`, `${packname}`, `📵 No Hacer Spam`, correo, `BVH3`, md, biobot]
    ], m)
}

handler.help = ["colaborador","owner"]
handler.tags = ["info"]
handler.command = ['colaborador','colab']
