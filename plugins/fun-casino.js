...
if (Aku > Kamu) {
conn.reply(m.chat, '`💙 Veamos que numeros tienen!`\n\n'+ `➠ *HATSUNE MIKU* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} 🌱 Cebollines.`.trim(), m)
} else if (Aku < Kamu) {
user.chocolates += count * 2
conn.reply(m.chat, '`💙 Veamos que numeros tienen!`\n\n'+ `➠ *HATSUNE MIKU* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *GANASTE* ${formatNumber(count * 2)} 🌱 Cebollines.`.trim(), m)
} else {
user.chocolates += count * 1
conn.reply(m.chat, '`💙 Veamos que numeros tienen!`\n\n'+ `➠ *HATSUNE MIKU* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username} obtienes ${formatNumber(count * 1)} 🌱 Cebollines.`.trim(), m, rcanal)}
} else conn.reply(m.chat, `No tienes *${formatNumber(count)} 🌱 Cebollines* para apostar!`.trim(), m)}
