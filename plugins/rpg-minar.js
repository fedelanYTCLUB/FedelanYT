...
let cebollines = `${pickRandom([20, 5, 7, 8, 88, 40, 50, 70, 90, 999, 300])}` * 1; let emerald = `${pickRandom([1, 5, 7, 8])}` * 1; let iron = `${pickRandom([5, 6, 7, 9, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80])}` * 1; let gold = `${pickRandom([20, 5, 7, 8, 88, 40, 50])}` * 1; let coal = `${pickRandom([20, 5, 7, 8, 88, 40, 50, 80, 70, 60, 100, 120, 600, 700, 64])}` * 1; let stone = `${pickRandom([200, 500, 700, 800, 900, 4000, 300])}` * 1
...
conn.fakeReply(m.chat, info, '0@s.whatsapp.net', '💙 *Minando.. - Hatsune Miku* 🌱', 'status@broadcast' )
await m.react('⛏️')

user.health -= 50
user.pickaxedurability -= 30
user.cebollines += cebollines
user.iron += iron
user.gold += gold
