...
let handler = async (m, { conn, args, text, usedPrefix }) => {
    ...
    if (!mentionedJid) return m.reply("💙 Debes mencionar a un usuario.")
    let name = (await conn.getName(mentionedJid)) || "Desconocido"
    let userData = global.db.data.users[mentionedJid]
    if (!userData || !userData.name) return m.reply(`💙 El usuario @${mentionedJid.split("@")[0]} no está registrado en la base de datos.`)
