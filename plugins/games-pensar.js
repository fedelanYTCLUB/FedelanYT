...
const handler = async (m, { conn }) => {
    ...
    if (pensandoUsuarios[senderId]) 
        return conn.reply(m.chat, '💙 Ya estás en una sesión de pensamiento. Envía un número del 1 al 100.', m, rcanal);
...
handler.before = async (m, { conn }) => {
    ...
    if (!/^\d+$/.test(texto) || texto < 1 || texto > 100) 
        return conn.reply(m.chat, '💙 Debes enviar un número válido del 1 al 100.', m, rcanal);
